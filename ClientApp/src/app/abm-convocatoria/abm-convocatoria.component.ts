...
export class AbmConvocatoriaComponent implements OnInit {
  ...
  ngOnInit() {
    this.servicio.loadGrilla('convocatoria').subscribe(res => this.convocatorias = res);
  }

  editar(id: number, obj: any) {
    if (obj != null && obj != undefined) {
      let con: convocatoria = new convocatoria
        (id.toString(), obj.fechainicio, obj.fechafin, obj.descripcion,
          obj.idmateria, obj.estado);
      this.abrirModal('Nuevo convocatoria', 'convocatoria', 3, con)
      ...
      let con: convocatoria = new convocatoria("0", "", "", "", "", "");
      this.abrirModal('Nuevo convocatoria', 'convocatoria', 3, con)
        .subscribe(obj => this.guardarconvocatoria(obj)
          .subscribe(json => this.servicio.loadGrilla('convocatoria')
            .subscribe(res => this.convocatorias = res)));
  ...
  guardarconvocatoria(obj): Observable<convocatoria> {
    if (+obj.id != 0) {
      let param: convocatoria =
        new convocatoria(obj.id, obj.fechainicio, obj.fechafin, obj.descripcion,
          obj.idmateria, obj.estado);
