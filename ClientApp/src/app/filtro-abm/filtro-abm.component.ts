     ...
     let n = 0;
      if (abm == "Reporte de asistencias") {
        for (let m in this.resultados[0]) {
          switch (m) {
            ...
            case "materia":
              head[0].push("Materia");
              break;
            case "modulosPresente":
              head[0].push("Módulos presente");
              break;
            case "cantidadModulos":
              head[0].push("Total módulos");
              break;
            default:
              break;
          }
        }
        head[0].push("Módulos ausente");
        head[0].push("Porcentaje de asistencia"); 
        ...
        for (var i in this.resultados) {
          ...
          ar.push(this.resultados[i]['cantidadModulos']-this.resultados[i]['modulosPresente']);
          ar.push((this.resultados[i]['modulosPresente']/this.resultados[i]['cantidadModulos']*100).toString().substring(0,5)+" %");
          body.push(ar);
        }
      }
      else {
