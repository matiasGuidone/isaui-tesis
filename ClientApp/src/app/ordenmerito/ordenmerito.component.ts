  ...
  aprobarConvocado() {
    ...
    this.servicio.adddoc(cv, materia, "altadocente").subscribe(r => {
      this.abrirModal("Tarea completada", "la selección se registro con éxito, el docente está dado de alta", 2, null).subscribe(e => {
        this.servicio.getById(materia.toString(), "materia").subscribe((mat: materia) => {
          this.servicio.getById(cv.toString(), "curriculum").subscribe(curr => {
            this.servicio.enviarcorreo("Felicidades !!!! es el nuevo docente de la materia <strong>" + mat.nombre + "</strong>", "Cierre de convocatoria", curr.correo).subscribe(cs => {
              let conv = this.convocatorias.find(convocatoria => convocatoria.id == document.getElementById('convocatoria')['value']);
              conv.estado = 2;
              this.servicio.addSingleAbm(conv,"convocatoria").subscribe(s=>{
                this.selecionarconvocatoria();
              })
