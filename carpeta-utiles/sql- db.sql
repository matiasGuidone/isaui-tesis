...
CREATE TABLE `test_isaui`.`asistencia` ( `id` INT NOT NULL AUTO_INCREMENT , `idestudiante` INT NOT NULL , `iddocente` INT NOT NULL , `idhoramateria` INT NOT NULL , `fecha` DATE NOT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;
...
ALTER TABLE `asistencia`
	ADD CONSTRAINT `fk_asistencia_estudiante` FOREIGN KEY (`idestudiante`) REFERENCES `estudiante` (`Id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
ALTER TABLE `calificacionestudiante` CHANGE `Idmateria` `Idciclolectivo` INT(11) NOT NULL;

ALTER TABLE `calificacionestudiante` ADD `Id` INT NULL AFTER `Idexamen`;

ALTER TABLE `examen` ADD `Idciclolectivo` INT NULL AFTER `tipo`;


CREATE TABLE `test_isaui`.`calificacionestudiante` ( `Idestudiante` INT NOT NULL , `Nota` INT NOT NULL , `Idexamen` INT NOT NULL ) ENGINE = InnoDB;

ALTER TABLE `calificacionestudiante` ADD `Id` INT NOT NULL AUTO_INCREMENT AFTER `Idexamen`, ADD PRIMARY KEY (`Id`);
